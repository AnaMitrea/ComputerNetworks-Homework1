...
int main()
{
    ...
    while (gets(cs), !feof(stdin)) 
    {
        ...
        {
            if ((num = write(fd1, "quit", strlen("quit"))) == -1) // se scrie in fifo si in num am cati bytes s-au scris
        ...
        if ((num = write(fd1, char_length, strlen(char_length))) == -1) // se scrie in fifo si in num am cati bytes s-au scris
            perror("[C] Problema la scriere in FIFO! \n");
