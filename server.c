                    ...
                    else //inseamna ca nu sunt logat => caut daca exista username
                    {
                        ...
                        */
                        ...
                        {  
                            ...
                            char numbytes_char[3];
                            int numbytes_int = 0;
                            
                            if (read(sockp[1], numbytes_char, strlen(numbytes_char)) < 0)  //cati bytes trimite copilul 
                                perror("[S] Eroare citire in parinte la socket!\n");

                            sscanf(numbytes_char, "%d", &numbytes_int);    // din char[] in int
                            char raspuns[numbytes_int + 1];
                            if (read(sockp[1], raspuns, strlen(raspuns)) < 0)  // raspunsul de la copil catre tata
                                perror("[S] Eroare citire in parinte la socket!\n");


                            printf("[S] Raspuns la login :  %s\n", raspuns);
                            

                            // trimit cati bytes are mesajul scris de server pt ca, CLIENTUL sa ii citeasca
                            if ((num2 = write(fd2, numbytes_char, 3)) == -1) // se scrie in fifo si in num am cati bytes s-au scris
                                perror("[S] Problema la scriere in FIFO! \n");
                            ...
                            else  // s-a deschis fisierul cu succes -> caut username-ul line by line
                            {
                                ...
                                while(fgets(name, maxlength, fis)) //citire linie cu linie din fisier 
                                {
                                    ...
                                    if(strcmp(name,user) == 0) // exista username ul in fisier => se scrie "V-ati logat cu succes!"
                                    {
                                        ...
                                        if (write(sockp[0], "22", sizeof("22")) < 0)   // cati bytes va citi tatal
                                            perror("[S] Eroare write la socket in copil!\n");

                                        
                                        if (write(sockp[0], "V-ati logat cu succes!", sizeof("V-ati logat cu succes!")) < 0)  // raspunsul la functia login
                                            perror("[S] Eroare write la socket in copil!\n");
                                        
                                        break;
                                    }
                                }
                                if(logat == 0) // nu s-a gasit username-ul in fisier
                                {
                                    if(strlen(user) > 0) // user valid dar nu exista in lista
                                    {
                                        if (write(sockp[0], "28", sizeof("28")) < 0)   // cati bytes va citi tatal
                                            perror("[S] Eroare write la socket in copil!\n");
                                        
                                        if (write(sockp[0], "Username-ul nu a fost gasit!", sizeof("Username-ul nu a fost gasit!")) < 0)  // raspunsul la functia login
                                            perror("[S] Eroare write la socket in copil!\n"); 
                                    }
                                    else
                                    if(strlen(user) > 30 || strlen(user) <= 0) // username invalid
                                    {
                                        if (write(sockp[0], "116", sizeof("116")) < 0)   // cati bytes va citi tatal
                                            perror("[S] Eroare write la socket in copil!\n");
                                        ...
                                        if (write(sockp[0], msg, sizeof(msg)) < 0)  // raspunsul la functia login
                                            perror("[S] Eroare write la socket in copil!\n"); 
                                    }
                                }
                            }
